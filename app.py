...
def get_recent_events_display():
    events = user_memory["events"][-10:]  # 最近10条
    if not events:
        return "暂无记录"
    lines = []
    for e in reversed(events):
        type_map = {"health":"🏥", "medication":"💊", "item":"🔑", "family":"👨‍👩‍👧‍👦", "habit":"🌱", "emotion":"❤️"}
        icon = type_map.get(e.get("type"), "📌")
        time_str = datetime.fromtimestamp(e["time"]).strftime('%m-%d %H:%M')
        lines.append(f"{icon} {e['content']} ({time_str})")
    return "\n".join(lines)
...
def text_to_speech(text):
    ...
    try:
        ...
        data = {
            "app": {"appid": VOLC_APP_ID, "token": "placeholder", "cluster": "volcano_tts"},
...
def chat_response(user_input, chat_history, surname, gender, is_muted, personality, conv_id, dropdown):
    ...
    save_conversation(conv_id, chat_history)
    # 如果是新对话且没有标题，异步生成标题
    if len(chat_history) <= 2:  # 只有开场和第一轮
...
def new_conversation():
    ...
    save_conversation(new_id, chat_history)
...
custom_css = """
    * { font-family: "Microsoft YaHei", sans-serif !important; }
    ...
    .big-btn { height: 52px !important; font-size: 18px !important; border-radius: 30px !important; background: #E3B87C !important; color: #4A3420 !important; font-weight: bold !important; border: none !important; min-width: 80px !important; }
    ...
    .event-box { background: #fff; border-radius: 12px; padding: 10px; margin-top: 10px; font-size: 14px; color: #4A3420; max-height: 200px; overflow-y: auto; }
...
with gr.Blocks(title="小忆陪伴助手") as demo:
    ...
    with gr.Column(visible=False) as main_view:
        with gr.Row():
            with gr.Column(scale=1, min_width=240):
                ...
                # 重要事件展示
                gr.Markdown("📝 **重要事件**")
                events_display = gr.Textbox(value=get_recent_events_display(), label="", lines=6, interactive=False, container=False, elem_classes="event-box")
                gr.HTML('</div>')
            with gr.Column(scale=4):
                ...
                mic_input = gr.Audio(sources="microphone", type="numpy", visible=False, label="麦克风输入")
                with gr.Row(elem_classes="input-area"):
                    mic_btn = gr.Button("🎙️ 语音", elem_classes="big-btn")
    ...
    # 麦克风
    mic_btn.click(lambda: gr.update(visible=True), outputs=mic_input)
    mic_input.stop_recording(process_mic, [mic_input, chat, surname, gender, is_muted, personality, current_conv_id, history_dropdown],
                             [txt, chat, audio_output, current_conv_id, history_dropdown]).then(lambda: gr.update(visible=False), outputs=mic_input).then(refresh_events_display, outputs=events_display)
